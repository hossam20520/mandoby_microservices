...
def create_user_role_seeds(db: Session, data):
    for items in data:
        try:
            db_user_role  = Role_userModel(**items)
            db.add(db_user_role)

        except IntegrityError:
            db.rollback()
            raise HTTPException(422, ResponseModel([] , "User_role  exist" , False , 422 , {"error":"Already exists"})) from None
    db.commit()             
