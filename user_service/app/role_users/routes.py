...
import app.role_users.models as models
import app.role_users.crud as crud 
...
@router.get("/seeds")
def create_roles_user_seeds( db: Session = Depends(get_db)  ):
    f = open('app/role_users.json')
    data = json.load(f)   
    return crud.create_user_role_seeds(db, data)
