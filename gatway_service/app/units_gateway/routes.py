...
from app.units_gateway.schemas import UnitCreate
router = APIRouter()



@router.get("/list")
def unit_list(skip: int = 0, limit: int = 100 , token: str =  Header()):
    auth = Auth()
    auth.accessToken(token)
    headers = auth.getHeaders()
    response = requests.get(f"http://product_service:8000/api/v1.0/units/pagentation" , headers=headers   )
